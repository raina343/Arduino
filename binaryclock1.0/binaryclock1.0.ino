...
char ssid[] = "Bi-naryClockSetup";  // your network SSID (name)
...
void setup() {
  ...
  if (owner.valid == false) {  // we get here when there's no saved data.  basically initial setup
    ...
    if (WiFi.status() == WL_NO_MODULE) {
      ...
      debugoutput(0, 1, 0, 0, 255, 0, 0, 1); //middle row red.  No Wifi Module.  this is very bad
      while (true)
        ;
    }
    ...
    //
    //        debugoutput(1, 0, 1, 0, 0, 150, 0, 1); //starting the wifi connection process  top and bottom row green
    //
    ...
    //
    //      debugoutput(0, 0, 0, 1, 0, 255, 0, 0); //dots green.  Wifi Connected
    //
    ////      printWifiStatus();
...
void SetTime() {
  ...
  */
  ...
  if ((String)owner.timezone == "IST") {
    ...
    time_t tIST = IIST.toLocal(alarmepoch, &tcr);
...
void loop() {
  ...
  if (WifiSetup == 0) {
    ...
    //light values for new photoresistor

    //4pm my house, just the back light on.. 720
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          String usewifi = "";
          if ((String)owner.usewifi == "of") {
            usewifi = " selected='selected' ";
          }
          ...
          client.println("<ul>");
          //           client.println("<Li>");
          //            client.println("<H3>If you set the 'Use Wifi' option to No the clock will pull your devices date/time and use that to set the clock.  time may drift by a small amount, and you will need to come back here and resave the settings to grab an updated time. </h3>");
          //        client.println("</Li>");
          //        client.println("<li>");
          //            client.println("<label for='usewifi'>Use Wifi for time</label>");
          //            client.println("<div class='select'>");
          //                client.println("<select name='usewifi' id='usewifi'>");
          //                    client.println("<option value='on' >on</option><option value='off' ");
          //                    client.print(usewifi);
          //                    client.print(">off</option></select><span class='focus'></span>");
          //            client.println("</div>");
          //        client.println("</li>");
          //          client.println("<Li>");
          //          client.println("<label for='ssid'>SSID</label>");
          client.println("<input id='ssid' style='display:none;' type='text' name='ssid'");
          client.print("value='nowifisupport' placeholder='ssid'>");
          //          client.println("</Li>");
          //          client.println("<li>");
          ...
          client.println("console.log(mypost);");
              ...
              while (name) {
                ...
                if (valu) {
                  ...
                  if (String(name) == "CurrentTime") {
                    ...
                    //rtc.adjust(DateTime( epoch + 4));
                    CurrentTime.toCharArray(owner.Password, 100);
                  }
                  ...
                  if (String(name) == "usewifi") {
                    String usewifi = valu;
                    usewifi.toCharArray(owner.usewifi, 3);
                  }
  ...
  rtc.setAlarm2(rtc.now(), DS3231_A2_PerMinute);
  rtc.setAlarm1(DateTime(0, 0, 0, 0, 0, 55), DS3231_A1_Second);
            client.println("Settings now being saved");
            client.println("HTTP/1.1 200 OK");
            client.println("Content-Type: text/html");
            client.println();
            ...
            //              Serial.println("Wifi Failed");
            //              status = WiFi.beginAP(ssid);
            //              debugoutput(1, 0, 1, 0, 255, 0, 0, 1); //no IP Address Returned.  This is bad
            //
            //
            //              // startupPersonalWAP();
            //              printWiFiStatus();
            ...
            debugoutput(1, 0, 1, 0, 0, 0, 255, 1); //validated Wifi.  normal operation
