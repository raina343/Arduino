...
Adafruit_NeoPixel UKstrip(LED_COUNT, UKLED_PIN, NEO_GRB + NEO_KHZ800);
int lightvalue;
...
void setup()
{
 ...
 rtc.setAlarmTime(0, 0, 59);
  ...
  strip.clear();
  UKstrip.begin();           // INITIALIZE NeoPixel strip object (REQUIRED)
  UKstrip.show();            // Turn OFF all pixels ASAP
  UKstrip.clear();
  strip.setBrightness(150); // Set BRIGHTNESS to about 1/5 (max = 255)
  UKstrip.setBrightness(150); // Set BRIGHTNESS to about 1/5 (max = 255)
  strip.setPixelColor(42, strip.Color(255, 255, 255));
  strip.setPixelColor(43, strip.Color(255, 255, 255));
  strip.show();
  UKstrip.setPixelColor(42, UKstrip.Color(255, 255, 255));
  UKstrip.setPixelColor(43, UKstrip.Color(255, 255, 255));
  UKstrip.show();
  getNTPTime();
}
...
//    //this should make sure the time is upto date every 15 mins
//    getNTPTime();
//  }
...
////  gettimeasstringdigits(rtc.getHours(), rtc.getMinutes(), rtc.getSeconds());
  ...
  if (lightvalue > 700) {
    ...
    strip.show();
    UKstrip.setBrightness(255);
    UKstrip.show();
  }
  if ((lightvalue > 450) && (lightvalue < 700)) {
    strip.setBrightness(150);
    strip.show();
    UKstrip.setBrightness(150);
    UKstrip.show();
  }
  if (lightvalue < 450) {
    strip.setBrightness(20);
    strip.show();
    UKstrip.setBrightness(20);
    UKstrip.show();
  }
...
//    minutevalue = rtc.getMinutes();
    ...
    outputdigit (PSTMinute2.toInt(), 0, 0, 0, 255);
    outputdigit (PSTMinute1.toInt(), 21, 0, 200, 0);
    outputdigit (PSTHour2.toInt(), 44, 255, 255, 0);
    outputdigit (PSTHour1.toInt(), 65, 255, 0, 0);

    strip.setPixelColor(42, strip.Color(150, 0, 150));
    strip.setPixelColor(43, strip.Color(150, 0, 150));
    strip.show();
    UKstrip.setPixelColor(42, UKstrip.Color(150, 0, 150));
    UKstrip.setPixelColor(43, UKstrip.Color(150, 0, 150));
    UKstrip.show();
...
//    delay(10000);
  ...
  strip.show();
//  delay(1000);
  minutecounter = 0; //reset the counter, so we always get the correct time every 24 hrs
  if (Udp.parsePacket()) {
...
void SetDateTime(Timezone tz, time_t utc, const char *descr) {
  ...
  strip.show();
  Serial.println('Setting Time to RTC');
