...
void setup() {
  ...
  if (owner.valid == false) {  // we get here when there's no saved data.  basically initial setup
    ...
    Serial.println((String)owner.wifi);
    WifiSetup = 0;
        ...
        if (c == '\n') {
          ...
          if (readString.indexOf("ssid") > 0) {
            ...
            unsigned long epoch;
            {
              ...
              while (name) {
                ...
                if (valu) {
                  ...
                  if (String(name) == "WifiTime") {
                    String temp = valu;
                    temp.toCharArray(owner.wifi, 3);
                    Serial.print("Wifi Time Setting");
                    Serial.println(temp);
            ...
            if ((String)owner.wifi == "of") {
              ...
              Serial.println("Setting Clock Time without Wifi");
              rtc.begin();  // initialize RTC
              rtc.disable32K();
              rtc.adjust(DateTime(epoch + 4));
              rtc.clearAlarm(1);
              ...
              rtc.writeSqwPinMode(DS3231_OFF);
              rtc.setAlarm1(rtc.now(), DS3231_A1_PerSecond);
              debugoutput(1, 1, 1, 0, 0, 0, 255, 1);  //validated Wifi.  normal operation
              owner.valid = true;
              my_flash_store.write(owner);
